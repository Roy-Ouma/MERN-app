...
const Banner = ({post} ) => {
    return (
     ...
     className='w-full '>
      ...
      bottom-10 md:bottom-2 w-full nd:w-2/4 lg:w-1/3  2xl:w-[480px] bg-white
      dark:bg-[#05132b] shadow-2xl rounded-lg gap-3 p-4'>
        <Link  to={`/${post?.slug}/${post?._id}`}>
          <h1 className='font-semibold text-2xl text-black dark:text-white
          '> {post?.title.slice(0,60) + "..."}</h1>
        </Link>
       ...
       text-sm text-justify '>
        ...
        <Link to ={`/writer/${post?.userId?._id}`} 
        className='flex gap-3 mt-4 items-centre'>
          <img src={post?.user?.image} alt="User-Profile"
          className='object-cover w-10 h-10 rounded-full'/>
          <span className='font-medium text-gray-700 dark:text-slate-500
          '>{post?.user?.name}</span>

          <span className='text-gray-500 dark:text-gray-600'> {new Date(post?.createdAt).toDateString()}</span>
